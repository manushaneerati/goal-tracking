...
// Collect goals that have progress == 100 to congratulate
$congratulations_goals = [];
foreach ($goals as $goal) {
    if ($goal['progress'] == 100) {
        $congratulations_goals[] = $goal['goal_text'];
    }
}
?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show congratulations message for goals at 100% progress
    const congratsGoals = <?php echo json_encode($congratulations_goals); ?>;

    if (congratsGoals.length > 0) {
        const goalsList = congratsGoals.map(g => `"${g}"`).join(', ');
        Swal.fire({
            icon: 'success',
            title: 'Congratulations!',
            html: `Your goal(s) reached 100% progress:<br><strong>${goalsList}</strong><br>Great job! 🎉`
        });
    }
});
</script>
